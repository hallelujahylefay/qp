...
for idx, mu in enumerate(mus):
    ...
    plt.savefig(f"lasso_qp_norm_{mu}", dpi=500)
    plt.close()
    ...
    plt.savefig(f"lasso_qp_lasso_objective_{mu}", dpi=500)
    plt.close()

for i in range(N):
    plt.plot(mus, wnorm[:,i])
plt.title(f"{N} lass problems w/ shape {shape[0], shape[1]}, w norm w.r.t mu")
plt.savefig(f"lass_qp_norm_mus", dpi=500)
plt.close()
...
plt.savefig(f"lass_qp_obj_mus", dpi=500)
plt.close()